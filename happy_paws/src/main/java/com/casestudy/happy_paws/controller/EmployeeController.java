...
public class EmployeeController {
    ...
    public String saveEmployee(@Validated @ModelAttribute(value = "employee") EmployeeDTO employeeDTO, BindingResult bindingResult,Model model,RedirectAttributes redirectAttributes) {
        if (bindingResult.hasErrors()) {
            ...
            Account account = new Account(employee.getAccount().getUsername(), employee.getAccount().getPassword(), new Role(2, "ROLE_EMPLOYEE"));
...
//            boolean check = true;
//            check = iEmployeeService.saveEmployee(employee);
//            redirectAttributes.addFlashAttribute("mess", true);
//            if (!check) {
...
//            }
    ...
    @PostMapping("/edit")
    public String updateBlog(@ModelAttribute Employee employee,RedirectAttributes redirectAttributes) {
        redirectAttributes.addFlashAttribute("mess",true);
        iEmployeeService.save(employee);
        return "redirect:/";
    }
